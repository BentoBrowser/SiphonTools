...
function randomCSSClass(): string {
    var text = "";
...
export function computedStyleToInlineStyle(element: HTMLElement, options: {recursive?: boolean; clone?: boolean | HTMLElement; properties?: false | string[]}): {element: HTMLElement; styleInfo: string} {
    ...
    if(clone) {
        if (typeof clone == "boolean") {
            clone = element.cloneNode(recursive) as HTMLElement;
        }
    }
    let styleInfo = "";
    ...
    if (recursive) {
        ...
        Array.prototype.map.call(children, function(child, idx): void {
            if (child.tagName == "IFRAME" && child.contentDocument && (clone as HTMLElement).children[idx]) { //Iframe case
                ...
                (clone as HTMLElement).replaceChild(placeholder, (clone as HTMLElement).children[idx])
            ...
            if (clone) {
                newOptions.clone = (clone as HTMLElement).children[idx] as HTMLElement;
            }
            if (!!child.style) {
    ...
    if (saveBefore) {
        ...
        for (var i = 0; i < computedStyle.length; i++) {
            ...
            if (!properties || properties.indexOf(property) >= 0) {
                ...
                //Only save values that aren't the initial ones for the element
                if (initialValues["::before"] &&  (initialValues["::before"] as {[key: string]: string})[property] != value) {
    ...
    if (saveAfter) {
        ...
        for (var i = 0; i < computedStyle.length; i++) {
            ...
            if (!properties || properties.indexOf(property) >= 0) {
                ...
                //Only save values that aren't the initial ones for the element
                if (initialValues["::after"] &&  (initialValues["::after"] as {[key: string]: string})[property] != value) {
                    if (property == "content") { //We need to escape content appropriately
                        var ret = '"'
                        for (let i = 1; i < value.length - 1; i++) {
                            let charCode = value.charCodeAt(i)
    ...
    if (clone) {
        ...
        if (clone.hasAttribute("src")) {
            clone.setAttribute("src", (element as HTMLImageElement).src)
        }

        if (clone.hasAttribute("href")) {
            clone.setAttribute("href", (element as HTMLLinkElement).href)
        }
...
export function getNodesToParents(elem: HTMLElement, clone: Element, parent: HTMLElement): {elems: HTMLElement[]; clones: HTMLElement[]; style: string} {
    let elems = [];
    let clones = [];
    let style = ""
    ...
    while (elem.parentElement != parent) {
        ...
        clone = element;
        elem = parent as HTMLElement;
    }

    return {elems, clones, style}
}

export function findProperParents(elem: HTMLElement, clone: HTMLElement): {elems: HTMLElement[]; clones: HTMLElement[]; style: string} {
    let elems = [elem];
    let clones = [clone];
    let style = ""
    ...
    //If we found the parent we want
        ...
        if (['li', 'dd', 'dt', //list elements
            ...
            if (parent) {
                ...
                element.appendChild(last(clones) as HTMLElement);
                elems.push(parent);
                clones.push(element);
                style += styleInfo
                elem = parent;
...
export function resolveHangingTags(originalNodes: HTMLElement[], cloneNodes: HTMLElement[]): {cloneNodes: HTMLElement[]; style: string}{
    ...
    for (let i = 0; i < cloneNodes.length; i++) {
        ...
        if (last(parents.clones)) {
            cloneNodes[i] = last(parents.clones) as HTMLElement;
        }
        style += parents.style
                ...
                if (parent.contains(originalNodes[j])) {
                    ...
                    if (Array.from(parent.children).includes(originalNodes[j])) {
                        ...
                        if (last(addParents.clones)) {
                            parents.clones[v].appendChild(last(addParents.clones) as HTMLElement);
                        }
