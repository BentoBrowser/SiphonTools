...
function resolveHangingTags(originalNodes: Element[], cloneNodes: Element[]) {
...
function getNodesToParents(elem: Element, clone: Element, parent) {
  let elems = [];
  let clones = [];
  let style = ""
...
function findProperParents(elem: Element, clone: Element) {
  let elems = [elem];
  let clones = [clone];
  let style = ""
...
function computedStyleToInlineStyle(element: Element, options = {recursive: true, clone: false, properties: false}) {
