...
export interface VideoSerializedAnnotation extends ElementSerializedAnnotation {
    src: string;
    video_id: string;
    provider: string;
    thumbnail: string;
...
function capture(video: HTMLVideoElement, scaleFactor: number): HTMLCanvasElement {
    if(scaleFactor == null){
        scaleFactor = 1;
    }
...
export default class Video extends ElementAnnotation{
    ...
    public serialize(): VideoSerializedAnnotation {
    ...
    public deserialize(serialized: VideoSerializedAnnotation): void {
