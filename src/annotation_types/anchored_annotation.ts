...
export default class AnchoredAnnotation extends BaseAnnotation {
  ...
  public refreshAnchorCoordinates() {
    if (this.anchor) {
  ...
  static closest(num: number, arr: HTMLElement[]) {
    let curr = arr[0];
  ...
  static filterLeafNodes({top, left, bottom, right}: {top: number, left: number, bottom: number, right: number},
    ...
    return uniq(filteredNodes.map((node) => node.elem as HTMLElement));
  }


  //Assume Child nodes will fill 90% of the area of their optimal parents
  static findOptimalParents(leafNodes: HTMLElement[]): HTMLElement[] {
    let parents = new Map();
    ...
    parents.forEach((children: HTMLElement[], parent: HTMLElement) => {
  ...
  //    if (childrenArea / parentArea >= 0.9) {
      ...
      if (Array.from(parent.children).filter(elem => visible(elem as HTMLElement)).length == children.length) {
        foundNewParent = true;
        newSet.push(parent);
      }
      else
